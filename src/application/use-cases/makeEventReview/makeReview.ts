...
export async function makeReview(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const newIReview: IReview = await ReviewRepository.addReview(
      newValoracioDTO.eventId,
      newValoracioDTO.authorId,
      newValoracioDTO.puntuation,
      newValoracioDTO.comment,
    );
    ...
    if (event) {
      ...
      const user = await UserRepository.findById(newIReview.authorId);
